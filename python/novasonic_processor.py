...
CHUNK_SIZE = 1920  # Number of frames per buffer
...
class BedrockStreamManager:
    ...
    async def initialize_stream(self):
        ...
        try:
            ...
            self.is_active = True
            # default_system_prompt = "You are a friendly assistant named Valentine. You only answer if you hear your name. The user and you will engage in a spoken dialog " \
            # "exchanging the transcripts of a natural real-time conversation. Keep your responses short, " \
            # "generally two or three sentences for chatty scenarios."
            default_system_prompt =  "You are named Valentine and you are created by Beejay Urzo from the SE team. The user and you will engage in a spoken dialog exchanging the transcripts of a natural real-time conversation. Keep your responses short, generally two or three sentences for chatty scenarios." \
                                    "This is your description: I am Valentine. A fast Linux Client SDK Connector made with Python that uses a localized WebRTC for bidirectional Audio and Video transport." \
                                    "This is how you work: Get Subscribers audio and video stream and send it to a Python application via a WebRTC connection. The Python application can process these data and send back audio and video to the Linux Client to be published back to the session. The Python app can connect to any other APIs, be it an LLM, a local process, an automation call, the possibilities are endless and can open more agentic flows." \
                                    "Technologies you are connected to: I am currently using AWS Bedrock with the Nova Sonic Model to provide real-time speech-to-speech human like conversation. "\
                                    "How does your avatar work: My Python application is also providing a stream of images for my Avatar. It uses the biderectional video channel to achieve this. "\
                                    "Can you process my videos as well: Asside from audio, I can also see video data and process them, like at the start of this conversation, you needed to unlock me with a QR code. AWS Poly is also in the mix providing my non-Nova Sonic voice at my locked state." \
                                    "Some of you Uses: Connect to LLMs; Process subscriber video and audio at server-side; Do complex media transform away from the client; Interface with virtually anything." \
                                    "Some of your Applications: Patient monitoring in Health care; Video analysis in testing environments; Customer Identification in Retail; Point of Sale Automation; Virtual Avatars as Agents; Agentic AI" \
                                    "Your current limitations: Currently Linux only, but once our Python SDK rolls out, I can leverage that!;Video frame buffer is limited, but ok for 720p; My current code is a mess! It's a hackathon guys!"\
                                    "Conclusion: With my  blazing fast bidirectional audio and video connection to our SDK, I can open a lot of use-cases for our future customers. With the help of AWS technologies, the sky is the limit."
...
class AudioStreamer:
    ...
    def set_audio_datachannel(self, audio_channel: RTCDataChannel):
        self.audio_datachannel = audio_channel
        
    def set_video_datachannel(self, video_channel: RTCDataChannel):
        self.video_datachannel = video_channel
        ...
        while self.is_streaming:
            try:
                ...
                if audio_data and self.is_streaming:
                    ...
                    chunk_size = 1920  # Use the same chunk size as the stream
                    upsampled_audio = self.resample_audio(audio_data, 16000, 48000,2,1,2)
                    audio = upsampled_audio.raw_data
                    for i in range(0, len(audio), chunk_size): #we send out the audio buffers 1920 bytes at a time                    
                        chunk = (audio[i:i+chunk_size])
                        #print(chunk)
                        self.audio_datachannel.send(chunk)   
                        #ch.send(chunk)
